...
export class InterestComponent implements OnInit {
  ...
  getInterests(): void {
    this.http.get<any[]>('http://localhost:3000/api/v1/interests')
      .subscribe(
        (response) => {
          this.interests = response.map(interest => ({ ...interest, selected: false }));
        },
        (error) => {
  ...
  toggleInterestSelection(interest: any): void {
    ...
    if (interest.selected) {
      ...
      const index = this.selectedInterests.findIndex(i => i.id === interest._id);
      if (index !== -1) {
  ...
  onSubmit(): void {
    ...
    this.http.post(`${this.url}${this.profileId}/interests`, 
